...
const char* httpFindHeader(const char* buf, size_t len, const char* name, size_t* valLen) {
    ...
    while (pos < len) {
        ...
        if (lineLen > nameLen && lineStart[nameLen] == ':' && strncasecmp(lineStart, name, nameLen) == 0) {
...
void httpParseHandshake(const char* buf, size_t len, HandshakeResult_t* out) {
    ...
    if (!upgrade || !httpHeaderHasToken(upgrade, upgradeLen, "websocket") || !connVal ||
        !httpHeaderHasToken(connVal, connLen, "Upgrade")) {
