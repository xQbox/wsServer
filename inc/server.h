...
#define DEFAULT_MAX_MESSAGE \
    (4u * 1024u * 1024u)           /* макс. размер собранного (возможно фрагментированного) сообщения */
...
#define UPSTREAM_HOST_MAX 256
#define UPSTREAM_PATH_MAX 512

typedef struct UpstreamConfig_t {
    char host[UPSTREAM_HOST_MAX];
    int port;
    char path[UPSTREAM_PATH_MAX];
} UpstreamConfig_t;
#
...
typedef struct ServerConfig_t {
    ...
    Agg_t* agg;
    UpstreamConfig_t* upstreams;
    size_t upstreamCount;
    size_t upstreamCapacity;
