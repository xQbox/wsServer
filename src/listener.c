...
/*
    Устанавливает файловый дескриптор в неблокирующий режим.
*/
...
/*
    Принимает все ожидающие подключения. Для каждого клиента создаёт Conn_t
    и регистрирует в epoll. EPOLLET + EPOLLONESHOT: после события fd
    "выключается" до epoll_ctl(MOD) — ровно этим держится инвариант "с
    соединением в любой момент работает не более одного потока".
*/
...
void *listenerThread(void *args)
{
    ...
    /* main закроет epollfd только после join() ВСЕХ потоков (см. inc/listener.h) */
    ...
    /* eventfd сигнала завершения — level-triggered (без EPOLLET), иначе при
       нескольких Listener'ах проснётся только один из них. */
    ev.events = EPOLLIN;
    ...
    while (!stop)
    {
        ...
        for (int i = 0; i < nfds; ++i)
        {
            ...
            /* EV_CONN: событие от клиента — передаём задачу в очередь как есть.
               Закрывать соединение листенеру больше не положено ни при каких
               флагах (включая EPOLLRDHUP/HUP/ERR): в буфере ещё может быть
               непрочитанный запрос, решение принимает владелец — воркер. */
